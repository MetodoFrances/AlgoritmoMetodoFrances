...
public:
    ...
    void cronogramaDePagos() {
        ...
        for (period; period <= totalCuotas; ++period)
           _calcularPeriodoDePago(saldoInicial,saldoFinal,cuotaFija,period,'S');
    }
private:
...
private:
    ...
    void _calcularPeriodoDePago(double& saldoInicial, double& saldoFinal, double& cuotaFija, uint64_t period, char tipoDeGracia) {
        saldoInicial = saldoFinal;
        double interes = saldoInicial * porcentajeTEP;
        if(tipoDeGracia == 'P') cuotaFija = interes;
        if(tipoDeGracia == 'T') cuotaFija = 0.00;
        double amortizacion = (tipoDeGracia == 'T')? 0.00: cuotaFija - interes;
        ...
        saldoFinal = (tipoDeGracia == 'T')? saldoInicial + interes:saldoInicial - amortizacion;
        ...
        if(tipoDeGracia == 'T' || tipoDeGracia == 'P') {
